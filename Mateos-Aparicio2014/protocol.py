...
class ExperimentalProtocol(object):
    ...
    def init_params(self, params):
        ...
        constraint_config = self.params_dict.get("Constraints", {})
    ...
    def run_iclamp(self, cell, target, tstop=10000.0):
        ...
        target_config = self.params_dict["Targets"]
        if self.target_namespace is not None:
            target_config = target_config[self.target_namespace]
    ...
    def run_vclamp(self, cell, target, tstop=10000.0):
        ...
        this_target_amp = this_target_config["V"][0] * this_target_config.get(
            "V_factor", 1.0
        )
        tstim = this_target_config.get("t", None)
        if tstim is None:
        ...
        vclamp_results = run_vclamp(cell, ts=[t0, t1, t2], amps=this_target_amp, tstop=tstop, v_init=self.v_hold)
