...
def detect_spikes(T, Y, t0, t1, before_peak=50.0, spike_height=-20.0):
    ...
    pre_peak_info = signal.find_peaks(
        Y[pre_period_idxs], height=spike_height, width=(None, int(before_peak / dt))
    )
    ...
    peak_info = signal.find_peaks(
        Y_spk, height=spike_height, width=(None, int(before_peak / dt))
    )
