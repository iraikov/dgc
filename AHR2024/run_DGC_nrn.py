...
def run_iclamp(
    ...
    if stim_site == 'soma':
        stim_sec=soma[0]
    elif stim_site == 'apical':
        stim_sec = apical[apical_index]
    else:
        raise RuntimeError(f"Unknown stimulation site {stim_site}")
    ...
    logger.info(f"dend ek = {apical[apical_index].ek} ena = {apical[apical_index].ena}")
...
@click.command()
@click.option("--apical-index", type=int, default=1, help="apical section number to record")
@click.option(
    "--config-path",
    "-c",
    required=True,
...
@click.option(
    "--stim-site", type=str, default='soma', help="site of stimulus current (soma or apical)"
)
