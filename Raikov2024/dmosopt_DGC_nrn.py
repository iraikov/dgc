...
@click.option("--optimizer", default=["nsga2"], type=str, multiple=True)
@click.option("--resample-fraction", default=1.0, type=float)
@click.option("--surrogate-method", default="gpr", type=str)
@click.option("--surrogate-n-iter", default=30000, type=int)
@click.option("--surrogate-batch-size", type=int)
@click.option("--save-surrogate-eval", is_flag=True)
@click.option("--gpytorch-cuda", is_flag=True)
@click.option(
    "--sensitivity/--no-sensitivity",
    default=False,
    help="enable parameter sensitivity settings",
)
@click.option("--sensitivity-method",
              type=click.Choice(['dgsm', 'fast'],
                                case_sensitive=True))
@click.option(
...
def main(
    ...
    if sensitivity and sensitivity_method is None:
    ...
    dmosopt_params = {
        ...
        "reduce_fun_name": "reduce_obj_results",
        "reduce_fun_module": "dmosopt_PR_nrn",
        "obj_fun_init_name": "make_obj_fun",
        "obj_fun_init_module": "dmosopt_PR_nrn",
        "obj_fun_init_args": {
        ...
        "optimizer_kwargs": tuple(( dict() for _ in range(len(optimizer)) )),
        ...
        "surrogate_kwargs": {"lengthscale_bounds": (1e-5, 100.0),
        ...
        "sensitivity_method_name": sensitivity_method,
